...
test.describe('Accessibility mode', () => {
  test('widget auto-solves identically to standard mode, backend returns 200', async ({ page }) => {
    blockCDN(page);
    ...
    // Find the AccessibilityDemo section
    const section = page.locator('section').filter({ hasText: '无障碍模式' });
    await expect(section).toBeVisible();

    // Wait for cap-widget to auto-solve
    ...
    harvestToken(token, 'accessibility');
  });
});
